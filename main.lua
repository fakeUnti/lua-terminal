...
while running do --actual code // add error codes for functions that can fail 
    ...
    elseif command == "mkdir" then --mkdir 
        ...
        goto continue
    elseif command == "dltapp" then --sudo remove app ADD SUPPORT FOR ARCH LINUX
        if useros == "unix" then
            if os.execute("command -v apt > /dev/null 2>&1") then
                os.execute("sudo apt remove " .. arg1)
            elseif os.execute("command -v pacman > /dev/null 2>&1") then --i use arch btw
                os.execute("sudo pacman -R " .. arg1)
            end
        else
            os.execute('powershell -Command "Get-AppxPackage ..."') --windows
        end
    end
